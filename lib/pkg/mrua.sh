...
pkg_install() {
    ...
    for bin in ikc xkc; do
        pkg_run cp -va "$p_source_dir/bin/$bin" "$bin_dst"
    done

    for bin in gbus_read_bin_to_file gbus_read_uint32; do
        pkg_run cp -va "$p_source_dir/MRUA_src/llad_smallapps/$bin" "$bin_dst"
    done

    for bin in rmfree rmmalloc; do
        pkg_run cp -va "$p_source_dir/MRUA_src/llad_xtest/$bin" "$bin_dst"
    done

    for lib in $build_libs; do
        pkg_run cp -va "$p_source_dir/MRUA_src/$lib/src/lib${lib}.so" "$lib_dst"
    done

    pkg_run cp -va "$p_source_dir/MRUA_src/gbuslib/src/libgbus.so" "$lib_dst"
    pkg_run cp -va "$p_source_dir/MRUA_src/rmoutput/rua/src/libruaoutput.so" "$lib_dst"

    local libs="llad rmchannel rmcw rmmm rmmm_g rmmm_t rua"

    for lib in $libs; do
        pkg_run cp -va "$p_source_dir/lib/lib${lib}.so" "$lib_dst"
    done
}
